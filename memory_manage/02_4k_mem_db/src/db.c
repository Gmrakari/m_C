...
int write_tmp_passwd_node_info_from_flash(uint8_t *ptr, uartp_tmp_passwd_t *data)
{
    ...
    memcpy(&old_num, ptr + offset, sizeof(uint16_t));

    cur_num = old_num + 1;
    memcpy(ptr + offset, &cur_num, sizeof(uint16_t));
...
int read_tmp_passwd_node_info_from_flash(uint8_t *ptr, uint16_t *passwd_num, uint8_t **passwd_list)
{
    if (!ptr || !passwd_num) {
        printf("[%s][%d]param invalid!\r\n", __func__, __LINE__);
        return -1;
