...
static void _test_01()
{
    ...
    uint8_t *frmae_ptr_2 = (uint8_t *)frame;
    
    memcpy(frmae_ptr_2 + sizeof(frame_t) - 1, data, data_len);

    // 帧包的长度 + 数据的长度 - (帧包中的checksum的长度 1)
    // uint16_t cal_checksum_len = sizeof(frame) + data_len - 1;
    uint16_t cal_checksum_len = sizeof(frame_t) - 1 + data_len;
    uint8_t checksum = _get_checksum((uint8_t *)frame, cal_checksum_len);

    memcpy(frmae_ptr_2 + sizeof(frame_t) - 1 + data_len, &checksum, sizeof(checksum));
