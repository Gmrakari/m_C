...
static void _test_02()
{
    ...
    memcpy(ptr + offset, &frame->version, sizeof(frame->version));
    offset += sizeof(frame->version);

    memcpy(ptr + offset, &frame->cmd, sizeof(frame->cmd));
    offset += sizeof(frame->cmd);
