...
void crc32_init(crc32_context_t* ctx) {
    ...
    ctx->crc = CRC32_POLY_INIT;
    ctx->table = crc32_table;
}
...
void crc32_update(crc32_context_t* ctx, const uint8_t* data, size_t length) {
    ...
    const uint32_t* table = ctx->table;
...
uint32_t crc32_finalize(crc32_context_t* ctx) {
    ...
    return ctx->crc ^ CRC32_POLY_XOROUT;
}
 ...
 * @return 计算得到的CRC32值
 */
static crc32_context_t ctx;
uint32_t calculate_crc32(const uint8_t* data, size_t length) {
    crc32_init(&ctx);
    crc32_update(&ctx, data, length);
    return crc32_finalize(&ctx);
}
